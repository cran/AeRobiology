...
#'@param export.result A \code{logical} argument. If \code{TRUE}, graphical resume of the quality control and pollen season parameters will be exported in your working directory and a new folder (\emph{"table_AeRobiology"}) will be created with a \code{xlsx} file inside including all parameters calculated from the definition of the pollen season. A more detailed information may be consulted in Details and \code{\link{calculate_ps}} function. The \code{export.result} argument will be \code{TRUE} by default.
#'@param export.format A \code{character} string specifying the file extension of the quality control graphical resume when its saved. Only valid if \code{export.result = TRUE}. The implemented options that can be used are: \code{"pdf", "png", "jpeg", "tiff"} or \code{"bmp"}. The argument will be \code{"pdf"} by default.
#'@param export.width A \code{numeric (double)} value specifying the width of the graph in inches. Only valid if \code{export.result = TRUE}. See \code{\link[ggplot2]{ggsave}} function for more details. The \code{export.wodth = TRUE} argument will be \code{10} by default.
#'@param export.height A \code{numeric (double)} value specifying the height of the graph in inches. Only valid if \code{export.result = TRUE}. See \code{\link[ggplot2]{ggsave}} function for more details. The \code{export.height} argument will be \code{10} by default.
...
#' @examples qualitycontrol(munich[,c(1:4)], export.result = FALSE)
...
qualitycontrol<-function(data,
  ...
  if(export.result == TRUE){ifelse(!dir.exists(file.path("plot_AeRobiology")), dir.create(file.path("plot_AeRobiology")), FALSE)}
  ...
  if(export.result==TRUE){
    graph
    ggsave(paste("plot_AeRobiology/QualityControl.", export.format, sep=""), plot=graph, width = export.width, height = export.height)
